...
public class User {
	...
	// SCM user build
	public static User fromSCMJson(JSONObject jsonObject) {
		User user = new User();
		user.setUrl(jsonObject.getString(USER_URL_FIELD));
		user.setHtmlUrl(jsonObject.getString(USER_HTML_URL_FIELD));
		user.setId(jsonObject.getInt(USER_ID_FIELD));
		user.setLogin(jsonObject.getString(USER_LOGIN_FIELD));
		user.setCreatedAt(Instant.parse(jsonObject.getString(CREATED_AT_FIELD)));
		user.setUpdatedAt(Instant.parse(jsonObject.getString(UPDATED_AT_FIELD)));

		if (!jsonObject.isNull(USER_EMAIL_FIELD)) {
			user.setEmail(jsonObject.getString(USER_EMAIL_FIELD));
		}
		if (!jsonObject.isNull(USER_COMPANY_FIELD)) {
			user.setCompany(jsonObject.getString(USER_COMPANY_FIELD));
		}
		if (!jsonObject.isNull(USER_LOCATION_FIELD)) {
			user.setLocation(jsonObject.getString(USER_LOCATION_FIELD));
		}
		return user;
